...
from sympy import trigsimp,symbols, sin, cos, shape, lambdify, Matrix, eye, zeros, diff
from math import pi
...
cons1=L*cos(q1)+L*cos(q1+q2)-q0
cons2=L*sin(q1)+L*sin(q1+q2)
cons=[cons1,cons2]
...
kane= KanesMethod(N,q_ind=[q0,q1,q2],u_ind=[u0,u1,u2],kd_eqs=kde)
...
mm_uncons= kane.mass_matrix
ff_uncons= kane.forcing

#mm=mm.subs({L:1,m:2,m0:1})
#mm_uncons=lambdify([q1,q2],mm)

Mi=mm_uncons[0,:]
fi=ff_uncons[0:1,0]
Md=mm_uncons[1:3,:]
fd=ff_uncons[1:3,0]
B=Matrix([[-1,L*cos(q1)+L*cos(q1+q2),L*cos(q1+q2)],[0,-L*sin(q1)-L*sin(q1+q2),-L*sin(q1+q2)]])
BT=B.transpose()
Bd=BT[1:3,0:2]
Bi=BT[0:1,0:2]
...
mm_halfq=(Mi-Bi.multiply(Bd.inv().multiply(Md))).col_join(B)
mm_halfq=mm_halfq.subs(subs_dict)
mm_half=lambdify([q1,q2],mm_halfq)

Bdot=time_derivative(B,N)
Bdot=Bdot.subs({q1d:u1,q2d:u2})
qdot_mat=Matrix([[u0],[u1],[u2]])
ff_halfq=(fi-Bi.multiply(Bd.inv().multiply(fd))).col_join(-Bdot.multiply(qdot_mat))
ff_halfq=ff_halfq.subs(subs_dict)
